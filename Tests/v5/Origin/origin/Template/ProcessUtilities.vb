...
#Region " Start stuff "
        ...
        ''' Starts a program
        Friend Shared Sub StartProcess(command As String, argument As String, useShellExecute As Boolean, log As Action(Of String))
            log($"Executing Command '{command} {argument}'")

            Using vProcess As Process = ProcessStart(command, argument, useShellExecute, log)
                If vProcess Is Nothing Then
                    log($"Command '{command} {argument}' failed")
                End If
            End Using
        End Sub
        ...
        ''' <param name="argument">Any arguments to the command</param>
        ''' <param name="useShellExecute">Indicates how to start the process</param>
        ''' <param name="delayInSeconds">Seconds to wait before start</param>
        ''' <param name="log">Adds possibility to inject logger method</param>
        ''' Starts a program delayed
        Friend Shared Sub StartProcess(command As String, argument As String, useShellExecute As Boolean, delayInSeconds As Integer, log As Action(Of String))
            Dim factory = New TaskFactory

            factory.StartNew(
                Sub()
                    Threading.Thread.Sleep(delayInSeconds * 1000)

                    log($"Executing Command '{command} {argument}' delayed '{delayInSeconds}' seconds")
        ...
        Friend Shared Function ProcessStart(command As String, argument As String, useShellExecute As Boolean, log As Action(Of String)) As Process
            Try
                ...
                If InStr(command, "\") > 0 Then
                    workingDir = Left(command, InStrRev(command, "\"))
                Else
                    workingDir = ""
                End If
...
#Region " Check processes "
        ...
        ''' <summary>
        ''' Check weather or not the specified ProcessID is running
        ''' </summary>
