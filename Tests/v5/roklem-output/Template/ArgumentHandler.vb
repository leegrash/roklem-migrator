...
Namespace Common
    ...
    ''' 
    ''' or:
    ''' 			_argumentHandler = New ArgumentHandler(Environment.GetCommandLineArgs())
    ''' 
...
#Region " Arguments "
        ...
        Public Sub Add(argument As Argument)
            If argument Is Nothing Then Throw New ArgumentNullException(NameOf(argument))
            ...
            Catch ex As Exception
                ...
                Console.WriteLine($"Duplicate Argument: {argument.ArgumentName}")
            End Try
        End Sub
                    ...
                    If Not String.IsNullOrWhiteSpace(arg) Then
                        If arg.StartsWith("/") Then
                            If arg.IndexOf(":"c) = 1 Then
                                Add(arg.Substring(1), "")
                            Else
                                Dim vValue = arg.Substring(0, arg.IndexOf(":"))
                                Add(vValue.TrimStart("/"c), arg.Substring(arg.IndexOf(":") + 1).Trim().Trim(""""))
                            End If
                        ElseIf arg.StartsWith("-") Then
                            ...
                            End If
                        Else
                            Add(cPath & pathNumber, arg.Trim().Trim(""""))
                            pathNumber += 1
                        End If
                    End If
                Next
...
' Changes made:
' - Ensured the compatibility with .NET 6.0 and removed any references to System.Windows.Forms.
' - Enhanced exception messages for better clarity.
' - Kept the original structure, ensuring that all command line argument parsing logic still works properly.