...
#Region " Start stuff "
        ...
        Friend Shared Sub ShellDoc(fileName As String, workingDirectory As String)
            ...
            Catch ex As Win32Exception
                If ex.NativeErrorCode = 1155 Then
                    ...
                    process.StartInfo.FileName = Environment.SystemDirectory & "\RUNDLL32.EXE"
                    process.StartInfo.Arguments = "shell32.dll,OpenAs_RunDLL """ & fileName & """" ' Ensure file names with spaces are handled
        ...
        Friend Shared Function ProcessStart(command As String, argument As String, useShellExecute As Boolean, log As Action(Of String)) As Process
            Try
                Dim workingDir As String = If(command.Contains("\"), command.Substring(0, command.LastIndexOf("\")), "")
...
#Region " Check processes "
        ...
        ''' </summary>
        ''' <returns>This application's ProcessID</returns>
        ...
        Friend Shared Function GetProcessIdFromFile(fileNameWithoutPath As String) As Integer
            If InStr(fileNameWithoutPath, ".") > 0 Then
                Dim matchingProcess() = Process.GetProcessesByName(Mid(fileNameWithoutPath, 1, InStrRev(fileNameWithoutPath, ".") - 1))
...
#Region " Kill "
        ...
        Friend Shared Function KillProcess(processId As Integer) As Boolean
            Try
                Using proc As Process = Process.GetProcessById(processId)
                    proc.Kill()
                End Using
                Return True
            Catch ex As Exception
...
'1. Added quotes around fileName in ShellDoc to handle file names with spaces.
'2. Used an existing method for command execution but ensured overall code integrity during migration.